...
def deal_history(uuid, base_model_index=None, user_model=None, lora_choice=None, style_model=None, deal_type="load"):
    ...
    if deal_type == "update":
        if (base_model_index is None) or (user_model is None) or (lora_choice is None) or (style_model is None and lora_choice == 'preset'):
            return gr.Gallery.update(value=[], visible=True), gr.Gallery.update(value=[],
...
def inference_input():
    with gr.Blocks() as demo:
        ...
        display_button.click(fn=launch_pipeline,
                             inputs=[uuid, pos_prompt, neg_prompt, base_model_index, user_model, num_images, lora_choice, style_model, multiplier_style,
                                     multiplier_human,
            ...
            with gr.Column():
                ...
                with gr.Row():
                    with gr.Column(scale=2):
                        ...
                        user_model_B = gr.Radio(label="第2个人物LoRA，按从左至右的顺序（2nd Character LoRA，counting from left to right）", choices=[], type="value",
                                                visible=False)
                    with gr.Column(scale=1):
                        update_button = gr.Button('刷新人物LoRA列表(Refresh character LoRAs)')
